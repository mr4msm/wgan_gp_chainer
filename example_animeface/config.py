...
class Generator(Chain):
    ...
    def __init__(self):
        ...
        with self.init_scope():
            self.fc0 = L.Linear(
                Z_VECTOR_DIM, 8 * BASE_CH * 8 * 8, nobias=False)
    ...
    def __call__(self, z):
        ...
        if isinstance(z, (np.ndarray, cuda.ndarray)):
            z = Variable(z)
        assert isinstance(z, Variable), type(z)

        x = F.reshape(self.fc0(z), (-1, 8 * BASE_CH, 8, 8))
        x = F.relu(self.bn0(x))
...
class Critic(Chain):
    ...
    def __init__(self):
        ...
        with self.init_scope():
            ...
            self.fc4 = L.Linear(8 * BASE_CH * 8 * 8, 1, nobias=True)
